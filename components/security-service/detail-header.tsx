import Link from "next/link";

import { logoMark } from "./security-service-data";

export function DetailHeader() {
  const navItems = ["Our Service", "How we work", "Customer Review", "About US"];

  return (
    <header className="border-b border-white/60 bg-white/90 backdrop-blur-sm">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-3">
          <img src={logoMark} alt="investigator ltd logo" className="h-12 w-auto" />
          <span className="sr-only">investigator ltd</span>
        </Link>

        <nav className="order-3 w-full overflow-x-auto pb-1 sm:order-none sm:w-auto sm:pb-0">
          <ul className="flex min-w-max items-center justify-center gap-1 sm:gap-2">
            {navItems.map((item) => (
              <li key={item}>
                <a
                  href="#services"
                  className="rounded-full px-4 py-2 text-sm font-medium tracking-[0.08em] text-[#015555] transition hover:bg-[#e7f4f1]"
                >
                  {item}
                </a>
              </li>
            ))}
          </ul>
        </nav>

        <div className="flex items-center gap-3">
          <a
            href="#"
            className="inline-flex h-10 items-center justify-center rounded-lg bg-slate-800 px-5 text-sm font-bold text-white transition hover:bg-slate-700"
          >
            Sign Up
          </a>
          <a
            href="#"
            className="inline-flex h-10 items-center justify-center rounded-lg bg-[linear-gradient(149deg,#0ba8dd_4%,#60d8ff_28%,#0ba8dd_56%)] px-5 text-sm font-bold text-white shadow-[0_8px_20px_-12px_rgba(11,168,221,0.85)] transition hover:brightness-105"
          >
            Login
          </a>
        </div>
      </div>
    </header>
  );
}
